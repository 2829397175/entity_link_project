...
class EncodeDataset(Dataset):
    def __init__(self, document_path, 
        ...
        self.seq_lens = {}
        self.use_entity_qids = use_entity_qids
        preprocess_path = os.path.join(document_path, 'preprocess_multiview')
        import spacy
        self.nlp = spacy.load("zh_core_web_sm")
        
        token_ids = self.tokenize_split_description(
            # "测试 用例罢了。。",
            # "为了过测试，别出bug了。。",
            "",
            "",
        ...
        #     self.samples, self.entity_title_to_id, self.qid_to_id = torch.load(os.path.join(preprocess_path, world + '.pt'))
        ...
        self.samples, self.entity_title_to_id, self.qid_to_id = self.load_entity_description(tokenizer, world, use_entity_qids)
        torch.save([self.samples, self.entity_title_to_id, self.qid_to_id], os.path.join(preprocess_path, world + '.pt'))
        
        self.entity_title_to_id = self.qid_to_id
        samples = []
        entity_title_to_id = {}
        qid_to_id = {}
        id_to_qid = {}
        for idx, sample in enumerate(zecheldataset.samples):
            qid = sample["label"]
            if qid in self.qid_to_id.keys():
                idx_ori =  self.qid_to_id[qid]
                assert idx_ori is not None
                sample_encode = self.samples[idx_ori]
            else:
                sample_encode = self.default_token_vec
                
            sample_encode["title"] = qid
            samples.append(sample_encode)
            entity_title_to_id[qid] = idx
            qid_to_id[qid] = idx
            id_to_qid[idx] = qid
            
        self.samples = samples
        self.qid_to_id = qid_to_id
        self.entity_title_to_id = entity_title_to_id
        self.id_to_qid = id_to_qid     
    def __len__(self):
    ...
    def load_entity_description(self, tokenizer, world, use_entity_qids):
        entity_desc = self.samples
        entity_title_to_id = self.entity_title_to_id
        qid_to_id = self.qid_to_id
        
        import pandas as pd
        corpus = pd.read_csv("/data/jiarui_ji/entity_link/knowledge_base/wp_title_desc_wd0315.tsv",
                    sep="\t",
                    names=["title","text"],
                    index_col=0,
                    encoding="utf-8")
        use_entity_qids = list(filter(lambda x: x in corpus.index, use_entity_qids))
        corpus = corpus.loc[use_entity_qids]
        num_lines = corpus.shape[0]

        sentence_nums = {}
        id = 0
        for idx, col_info in tqdm(enumerate(corpus.iterrows()),total=num_lines):
        ...
        return entity_desc, entity_title_to_id,qid_to_id
    ...
    def tokenize_split_description(self, title, desc, tokenizer):
        ...
        ents = [ent.text for ent in self.nlp(desc.replace(' .', '.')).ents]
        if len(ents) == 0:
            ents = desc.split(" ")
...
class ZeshelDataset(Dataset):
    def __init__(self, 
        ...
        q_ids = [s["label"] for s in self.samples]
        self.entity_desc = {
    ...
    def get_qid_from_title(self,title):
        assert title in self.entity_desc["test_world"].entity_title_to_id.keys()
        idx = self.entity_desc["test_world"].entity_title_to_id[title]
        qid = self.entity_desc["test_world"].id_to_qid.get(idx,"NULL")
        return qid
    ...
    def load_training_samples(self, dataset_path, mode, max_seq_len):
        ...
        if mode == "train":
            num_lines_sampled = 30000
        else:
            num_lines_sampled = None
        ...
        if num_lines_sampled is not None and len(self.samples) < num_lines_sampled:
            data_path = os.path.join(dataset_path, "{}.jsonl".format(mode))
            
            num_lines = sum(1 for line in open(data_path, 'r'))
            if num_lines_sampled is None:num_lines_sampled = num_lines
            print("Set/{}: preprocessing {} samples".format(mode, num_lines_sampled - len(self.samples)))    

            with open(data_path, 'r') as sample_f:
                idx = 0
                for sample_line in tqdm(sample_f, total = num_lines_sampled):
                    if idx < len(self.samples):
                        idx += 1
                        continue
                    try:
        ...
        if mode == "train" and len(self.samples) > num_lines_sampled:
            self.samples = self.samples[:num_lines_sampled]
    ...
    def __getitem__(self, idx):
        ...
        label, world = sample['label'], "test_world"
        label_int_id = self.entity_desc[world].qid_to_id[label]
        assert isinstance(label_int_id,int)
        if self.all_sentences:
            return {
                "label_ids": [-1],
                "context_ids": context_ids,
                "world": world,
                "label_world_idx": label_int_id
            }
        else:
            return {
                "label_ids": self.entity_desc[world][label_int_id]['token_ids'],
                "context_ids": context_ids,
                "world": world,
                "label_world_idx": label_int_id
            }
...
def cross_collate_fn(batch):
    world = ["test_world" for sample in batch]
